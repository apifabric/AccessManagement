...
resourcename="accessmanagement"  # lower case, only
resourcegroup="accessmanagement_rg"
...
# git clone https://github.com/apilogicserver/accessmanagement.git
# cd accessmanagement
...
az group create --name accessmanagement_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group accessmanagement_rg --sku S1 --is-linux
az container create --resource-group accessmanagement_rg --name accessmanagement --image apilogicserver/accessmanagement:latest --dns-name-label accessmanagement --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//accessmanagement.westus.azurecontainer.io:5656'
