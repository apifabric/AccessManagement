...
def declare_logic():
    ...
    # Permanent accesses for special users or owners are automatically validated through is_permanent property.
    None required, as boolean is validated by default property.

    # Ensure access is only given within start and end date of a temporary invitation.
    Rule.constraint(validate=Invitation, as_condition=lambda row: row.start_date <= current_date() <= row.end_date, error_msg='Access only valid on specified dates.')

    # Allow recurring invitation logic for specific schedules.
    Rule.formula(derive=Invitation.is_within_schedule, as_expression=lambda row: recurring_logic_check(row))
